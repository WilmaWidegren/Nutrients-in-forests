...
def grow_initial_forest(Number_of_trees , max_size):
    tree_size = []
    for i in range(Number_of_trees):
        tree_size.append(np.random.uniform(1, max_size))
    return np.array(tree_size)
    

def calculate_tree_growth(x :float, k: float, A: float, m: float, C: float) -> np.ndarray:
    """
    ...
    Returns:
    - np.ndarray: Calculated tree growth (Y) for each size (x).
    """
    
    # Ensure all inputs are float for calculation
    k, A, m = float(k), float(A), float(m)
...
def no_mycorrhiza(NUM_TREES, T, initial_forest_size):
    ...
    ### Main ###
    while time < T:
        next_size = calculate_tree_growth(forest_size, k, A, m, carbon)
